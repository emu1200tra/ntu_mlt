...
def adaboost(data , datax , datay , T , test_data , test_datax , test_datay):
  N = float(len(datax))
  ...
  for i in range(T):
    ...
    for j in range(len(datax)):
      if(int(datay[j]) != hypothesis(s , feature , theta , datax[j][feature])):
        ...
        U[j] /= diamond
    for j in range(len(test_datax)):
      if(int(test_datay[j]) != hypothesis(s , feature , theta , test_datax[j][feature])):
        error_record += 1 

    alpha.append(math.log(diamond))
    Eout.append(float(error_record / len(test_datax)))

  return alpha , s , feature , theta , parameter , Eout

def G(alpha , s , feature , theta , x , T , parameter):
  counter = 0.0
  for i in range(T):
    counter += alpha[i]*hypothesis(parameter[i][0] , parameter[i][1] , parameter[i][2] , x[feature])
  return sign_my(counter)


data , datax , datay = readin_data(train_filename)
test_data , test_datax , test_datay = readin_data(test_filename)
alpha , s , feature , theta , parameter , Eout = adaboost(data , datax , datay , T , test_data , test_datax , test_datay)
plt.plot(Eout)
plt.show()
